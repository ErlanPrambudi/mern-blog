...
const UpdateManagement = () => {
  ...
  useEffect(() => {
    try {
      const fetchPost = async () => {
        const res = await fetch(`/api/management/getmanagements?managementId=${managementId}`);
        ...
        if (res.ok) {
          ...
          setFormData(data.managements[0]);
          return;
        }
      };
      fetchPost();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`/api/management/updatemanagement/${formData._id}/${currentUser._id}`, {
        method: "PUT",
        headers: {
  ...
  return (
    <div className="p-3 mx-auto min-h-screen   ">
      <h1 className="text-center text-3xl my-7 font-semibold">Update a management</h1>
